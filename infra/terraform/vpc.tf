# VPC — 2 AZs, public + private subnets, single NAT GW (upgrade to 2 for HA prod)
...
# ── NAT Gateway (single — cost-optimised MVP; add second for HA) ─────────────
...
resource "aws_security_group" "api" {
  ...
  description = "ECS API: inbound from ALB only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "worker" {
  ...
  description = "ECS workers: no inbound, outbound to Redis/RDS/S3/LLM APIs"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "redis" {
  ...
  description = "ElastiCache: inbound from API and workers only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "RDS: inbound from API and workers only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "qdrant" {
  name        = "${local.name_prefix}-sg-qdrant"
  description = "Qdrant ECS: inbound from API and workers only"
  vpc_id      = aws_vpc.main.id
  ...
  tags = { Name = "${local.name_prefix}-sg-qdrant" }
}

resource "aws_security_group" "efs" {
  name        = "${local.name_prefix}-sg-efs"
  description = "EFS: inbound NFS from Qdrant task only"
  vpc_id      = aws_vpc.main.id
